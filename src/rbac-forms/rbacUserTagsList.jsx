...
const renderTags = tags =>
  tags.map(({ name, value }) => (
    <Fragment key={name}>
      <Icon type="fa" name="tag" /> {`${name}: ${value}`}
      <br />
    </Fragment>));
  ...
  <div>
    ...
    <div className="form-horizontal static">
      <div className="form-group">
        <label className="col-md-2 control-label"> { /* eslint-disable-line jsx-a11y/label-has-for */ }
        ...
        <div className="col-md-8">
          {tags && tags.length > 0 ?
            renderTags(tags) : (
              <span>
                <Icon type="fa" name="tag" /> {sprintf(__('No %s Tags have been assigned'), tenant)}
              </span>)
          }
        </div>
      </div>
